...
uint64_t StreamPrefetcher::access(MemReq& req) {
    ...
    if (req.type != GETS && req.type != GETX) //modified by WeiWu on 250901 to test when including stores requests;
            ...
            if (e.conf.pred()) {  // do prefetches
                ...
                if (prefetchPos < 64 && !e.valid[prefetchPos]) {
                    ...
                    MemReq pfReq = {req.lineAddr + prefetchPos - pos,
                        (req.type == GETX ? GETX : GETS),  // modified by WeiWu on 250901 to test when including stores requests;
                        req.childId, &state, reqCycle, req.childLock,
                        state, req.srcId, MemReq::PREFETCH, req.pc};
