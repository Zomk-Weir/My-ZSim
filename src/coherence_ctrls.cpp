...
#include "cache.h"
#include "network.h"
// modified by wei wu on 250816: Added cache miss recorder include for MissType
#include "cache_miss_recorder.h"
// Added fake hit manager for PC-specific fake hits
#include "fake_hit_manager.h"
#include "filter_cache.h"

//uncomment next line to enable the profiling of the memory footprint, modified by wei on 251203
// #define PROFILING_MEMORY_FOOTPRINT 
...
uint64_t MESIBottomCC::processAccess(Address lineAddr, uint32_t lineId, AccessType type, uint64_t cycle, uint32_t srcId, uint32_t flags, Address pc) {
    ...
    switch (type) {
        ...
        case GETS:
            if (*state == I) {
                // // Check for fake hit before processing actual miss 1st edit
                // if (g_fakeHitManager && g_fakeHitManager->shouldFakeHit(pc) && parentCache) {
                //     std::string cacheName = parentCache->getName();
                //     if (g_fakeHitManager->isL2Cache(cacheName) || g_fakeHitManager->isL3Cache(cacheName)) {
                //         // Perform fake hit: transition to S state without accessing parent
                //         // *state = S;
                //         // profGETSHit.inc();  // Count as hit instead of miss
                //         // g_fakeHitManager->recordFakeHit(cacheName, pc, MISS_GETS);
                //         // info("[FAKE HIT] %s: GETS fake hit for PC 0x%lx", cacheName.c_str(), pc);
                //         // break;  // Skip actual miss processing
                //     }
                // }
                
                // Original miss processing
                ...
                // respCycle += nextLevelLat + netLat; //end of original miss processing

                // Fake hit processing 2nd edit
                if (g_fakeHitManager && g_fakeHitManager->shouldFakeHit(pc) && parentCache) {
                    std::string cacheName = parentCache->getName();
                    if (g_fakeHitManager->isL1Cache(cacheName) || g_fakeHitManager->isL2Cache(cacheName) || g_fakeHitManager->isL3Cache(cacheName)) {  //L1$ fake hit
                    // if (g_fakeHitManager->isL2Cache(cacheName) || g_fakeHitManager->isL3Cache(cacheName)) {  //L2$ fakehit
                    // if (g_fakeHitManager->isL3Cache(cacheName)) {   //L3$ fakahit 
                        // Perform fake hit: transition to S state without accessing parent
                        // *state = S;
                        // profGETSHit.inc();  // Count as hit instead of miss
                        g_fakeHitManager->recordFakeHit(cacheName, pc, MISS_GETS);
                        // info("[FAKE HIT] %s: GETS fake hit for PC 0x%lx", cacheName.c_str(), pc);
                        break;  // Skip actual miss processing
                        //do nothing for respCycle under fake hit
                    }
                    else{
                ...
                // modified by wei wu on 250816: Record cache miss
                if (parentCache && parentCache->getMissRecorder()) {
                    parentCache->getMissRecorder()->recordMiss(pc, MISS_GETS, cycle, lineAddr);
                }
        ...
        case GETX:
            if (*state == I || *state == S) {
                ...
                //     std::string cacheName = parentCache->getName();
                //     if (g_fakeHitManager->isL2Cache(cacheName) || g_fakeHitManager->isL3Cache(cacheName)) {
                //         // Perform fake hit: transition to M state without accessing parent
                //         MissType missType = (*state == I) ? MISS_GETXIM : MISS_GETXSM;
                //         // *state = M;
                //         // profGETXHit.inc();  // Count as hit instead of miss
                //         g_fakeHitManager->recordFakeHit(cacheName, pc, missType);
                //         // info("[FAKE HIT] %s: GETX fake hit for PC 0x%lx (type: %s)", 
                //         //      cacheName.c_str(), pc, MissTypeName(missType));
                //         break;  // Skip actual miss processing
                //     }
                // }
                
                //Profile before access, state changes
                MissType fakeHitType = MISS_GETXIM;
                if (*state == I) {
                    profGETXMissIM.inc();
                    
                    if (g_fakeHitManager && g_fakeHitManager->shouldFakeHit(pc) && parentCache) {
                        fakeHitType = MISS_GETXIM;
                    }

#ifdef PROFILING_MEMORY_FOOTPRINT
                    if(pc == 0x4072e6 || pc == 0x4072e2 || pc == 0x4072d0 || pc == 0x4072d4 || pc == 0x4072d9 || pc == 0x4072de || pc == 0x4072e3 || pc ==0x4072c0 || pc ==0x4072ce || pc ==0x407400) {
                        std::string cacheName = parentCache->getName();
                        info("[%s] mGETXIM of 0x%lx, issue cycle: %ld", cacheName.c_str(), pc, cycle);
                        if((pc==0x4072e6||pc==0x4072e3 || pc==0x407400) && cacheName=="l3-0") {
                            movaps_ins_count++;
                        }
                    }
#endif
                    
                    // modified by wei wu on 250816: Record cache miss for I->M
                    if (parentCache && parentCache->getMissRecorder()) {
                        parentCache->getMissRecorder()->recordMiss(pc, MISS_GETXIM, cycle, lineAddr);
                    } 
                    ...
                    if (g_fakeHitManager && g_fakeHitManager->shouldFakeHit(pc) && parentCache) {
                        fakeHitType = MISS_GETXSM;
                    }

                    // modified by wei wu on 250816: Record cache miss for S->M
                    if (parentCache && parentCache->getMissRecorder()) {
                        parentCache->getMissRecorder()->recordMiss(pc, MISS_GETXSM, cycle, lineAddr);
                    } 
                ...
                if (g_fakeHitManager && g_fakeHitManager->shouldFakeHit(pc) && parentCache) {
                    std::string cacheName = parentCache->getName();
                    if (g_fakeHitManager->isL1Cache(cacheName) || g_fakeHitManager->isL2Cache(cacheName) || g_fakeHitManager->isL3Cache(cacheName)) {   //L1 $ fakahit
                    // if (g_fakeHitManager->isL2Cache(cacheName) || g_fakeHitManager->isL3Cache(cacheName)) {  //L2$ fakehit
                    // if (g_fakeHitManager->isL3Cache(cacheName)) {   //L3$ fakahit
                        
                        // profGETXHit.inc();  // Count as hit instead of miss
                        g_fakeHitManager->recordFakeHit(cacheName, pc, fakeHitType);
                        // info("[FAKE HIT] %s: GETX fake hit for PC 0x%lx (type: %s)", 
                        //      cacheName.c_str(), pc, MissTypeName(missType));
                        break;  // Skip actual miss processing
                        //do nothing for respCycle under fake hit
                ...
                if(pc == 0x4072e6 || pc == 0x4072e2 || pc == 0x4072d0 || pc == 0x4072d4 || pc == 0x4072d9 || pc == 0x4072de || pc == 0x4072e3 || pc ==0x4072c0 || pc ==0x4072ce || pc ==0x407400) {
                    std::string cacheName = parentCache->getName();                    
                    info("[%s] hGETX of 0x%lx, issue cycle: %ld", cacheName.c_str(), pc, cycle);
                    if(pc==0x4072e6||pc==0x4072e3||pc==0x4072ce||pc ==0x407400) {
                        movaps_ins_count++;
                    }
                }
#endif
