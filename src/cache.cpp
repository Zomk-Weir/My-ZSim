...
#include "zsim.h"
// modified by wei wu on 250816: Added coherence_ctrls include for dynamic_cast
...
Cache::Cache(uint32_t _numLines, CC* _cc, CacheArray* _array, ReplPolicy* _rp, uint32_t _accLat, uint32_t _invLat, const g_string& _name)
    ...
    // modified by wei wu on 250816: Initialize cache miss recorder
    missRecorder = nullptr;
    if (g_cacheMissRecorderManager) {
        missRecorder = g_cacheMissRecorderManager->getRecorder(_name.c_str());
        if (missRecorder) {
            // info("[%s] Cache miss recorder initialized successfully", _name.c_str());
        } else {
            info("[%s] Cache miss recorder not created (recording disabled)", _name.c_str());
        }
    } else {
        info("[%s] Global cache miss recorder manager not initialized", _name.c_str());
    }
    
    // modified by wei wu on 250816: Set cache reference in coherence controller
    if (MESICC* mesiCC = dynamic_cast<MESICC*>(_cc)) {
        mesiCC->setParentCache(this);
        // info("[%s] Set cache reference in MESI coherence controller", _name.c_str());
    } else if (MESITerminalCC* terminalCC = dynamic_cast<MESITerminalCC*>(_cc)) {
        terminalCC->setParentCache(this);
        // info("[%s] Set cache reference in MESI terminal coherence controller", _name.c_str());
    } else {
        info("[%s] Coherence controller is not MESI type", _name.c_str());
    }
}
