...
#include "zsim.h"
// modified by wei wu on 250816: Added cache miss recorder include
#include "cache_miss_recorder.h"
// Added fake hit manager include
...
// modified by wei: gather support for vpgatherdq (AVX2 256-bit, 4 x int32 indices -> 4 x int64 loads)
// Called IPOINT_BEFORE the gather instruction. Injects 4 load addresses into the
// loadAddrs[] pipeline via IndirectPredLoadSingle, one per lane.
// active lane  (mask sign bit == 1): real element address -> load(addr, pc, true)
// inactive lane (mask sign bit == 0): predFalseLoad()    -> load(-1L, pc, false)
// This keeps loadIdx == loads invariant in OOOCore::bbl() satisfied, since
// decoder.cpp emits exactly 4 UOP_LOAD uops for this instruction.
VOID GatherDQ256LoadElements(THREADID tid, ADDRINT pc,
                              ADDRINT baseAddr,
                              PIN_REGISTER* idxReg,   // xmm: 4 x int32 gather indices
                              PIN_REGISTER* maskReg,  // ymm: 4 x int64, sign bit = active
                              UINT32 scale) {
    for (int i = 0; i < 4; i++) {
        BOOL active = ((int64_t)maskReg->qword[i] < 0);  // sign bit 1 = lane active
        ADDRINT elemAddr = 0;
        if (active) {
            int32_t idx = (int32_t)idxReg->dword[i];
            elemAddr = baseAddr + (ADDRINT)((int64_t)idx * (int64_t)(int32_t)scale);
        }
        // Reuse predicated load path: active=true -> load(elemAddr), false -> -1L
        IndirectPredLoadSingle(tid, elemAddr, pc, active);
    }
}
...
VOID Instruction(INS ins) {
    ...
    if (!procTreeNode->isInFastForward() || !zinfo->ffReinstrument) {
        ...
        // Detect gather/scatter: IARG_MEMORYREAD_EA is invalid for these instructions
        // (Pin does not support it for non-standard multi-address memory ops).
        // We handle them separately below with a dedicated callback.
        xed_category_enum_t insCategory = (xed_category_enum_t) INS_Category(ins);
        bool isGather = (insCategory == XED_CATEGORY_GATHER ||
                         insCategory == XED_CATEGORY_AVX2GATHER);

        if (INS_IsMemoryRead(ins) && !isGather) {
        ...
        if (INS_HasMemoryRead2(ins) && !isGather) {
            if (!INS_IsPredicated(ins)) {
        ...
        // vpgatherdq: 256-bit, 4 x int32 indices -> 4 x int64 loads
        // Inject 4 load addresses (one per lane, respecting mask) into loadAddrs[].
        // decoder.cpp emits 4 UOP_LOAD for this instruction to keep the 1:1 invariant.
        if (isGather && (xed_iclass_enum_t)INS_Opcode(ins) == XED_ICLASS_VPGATHERDQ) {
            REG baseReg = REG_INVALID(), idxReg = REG_INVALID(), maskReg = REG_INVALID();
            UINT32 scale = 1;

            // Extract base/index/scale from the single memory operand (compile-time info)
            for (uint32_t i = 0; i < INS_OperandCount(ins); i++) {
                if (INS_OperandIsMemory(ins, i)) {
                    baseReg = INS_OperandMemoryBaseReg(ins, i);
                    idxReg  = INS_OperandMemoryIndexReg(ins, i);
                    scale   = INS_OperandMemoryScale(ins, i);
                    break;
                }
            }
            // Find mask register: the ymm reg that is both read and written (not the dst)
            REG dstReg = REG_INVALID();
            for (uint32_t i = 0; i < INS_OperandCount(ins); i++) {
                if (INS_OperandIsReg(ins, i) && INS_OperandWritten(ins, i)) {
                    dstReg = REG_FullRegName(INS_OperandReg(ins, i));
                    break;
                }
            }
            for (uint32_t i = 0; i < INS_OperandCount(ins); i++) {
                if (INS_OperandIsReg(ins, i) &&
                    INS_OperandRead(ins, i) && INS_OperandWritten(ins, i)) {
                    REG r = REG_FullRegName(INS_OperandReg(ins, i));
                    if (r != dstReg) { maskReg = r; break; }
                }
            }

            if (REG_valid(baseReg) && REG_valid(idxReg) && REG_valid(maskReg)) {
                INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)GatherDQ256LoadElements,
                    IARG_THREAD_ID,
                    IARG_INST_PTR,
                    IARG_REG_VALUE,     REG_FullRegName(baseReg),
                    IARG_REG_REFERENCE, REG_FullRegName(idxReg),
                    IARG_REG_REFERENCE, REG_FullRegName(maskReg),
                    IARG_UINT32, scale,
                    IARG_END);
            } else {
                warn("vpgatherdq at 0x%lx: could not identify base/idx/mask registers, skipping gather instrumentation", (uint64_t)INS_Address(ins));
            }
        }
    ...
    if (INS_IsXchg(ins) && INS_OperandReg(ins, 0) == REG_RCX && INS_OperandReg(ins, 1) == REG_RCX) {
        ...
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR) HandleMagicOp, IARG_THREAD_ID, IARG_REG_VALUE, REG_RCX, IARG_END); 
        // modified by wuwei: use REG_RCX instead of REG_ECX, b/c ECX only contains low 32 bits of RCX register
    }
...
VOID SimEnd() {
    ...
    if (procIdx == 0) {
        ...
        // modified by wei wu on 250816: Export cache miss records
        if (g_cacheMissRecorderManager && g_cacheMissRecorderManager->isEnabled()) {
            info("Exporting cache miss records");
            g_cacheMissRecorderManager->exportAllToHDF5(zinfo->outputDir);
        }
        
        // modified by wei wu on 251211: Export PC access records
        if (g_pcAccessRecorderManager && g_pcAccessRecorderManager->isEnabled()) {
            info("Exporting PC access records");
            g_pcAccessRecorderManager->exportAllToHDF5(zinfo->outputDir);
        }
        
        // Print fake hit statistics
        if (g_fakeHitManager) {
            info("Printing fake hit statistics");
            g_fakeHitManager->printStatistics();
        }
...
VOID HandleMagicOp(THREADID tid, ADDRINT op) {
    switch (op) {
        ...
        case ZSIM_MAGIC_OP_ROI_END:
            if (!zinfo->ignoreHooks) {
                ...
                if (procTreeNode->getSyncedFastForward()) {
                    ...
                    // warn("Ignoring ROI_END magic op, already in fast-forward"); //temporarily ignore this warning
                }
                futex_unlock(&zinfo->ffLock);
            }
            return;
        ...
        default:
            ...
            // info("OVEC instruction detected: op=0x%llx", (unsigned long long)op);// data format matching the one in carribot.cpp
