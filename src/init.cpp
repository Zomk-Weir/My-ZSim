...
void SimInit(const char* configFile, const char* outputDir, uint32_t shmid) {
    ...
    zinfo->cacheMissRecordingEnabled = config.get<bool>("sim.cacheMissRecording.enabled",false);
    zinfo->cacheMissMaxRecords = config.get<uint64_t>("sim.cacheMissRecording.maxRecords", 20000000UL);
    
    info("Cache miss recording configuration: enabled=%s, maxRecords=%lu", 
         zinfo->cacheMissRecordingEnabled ? "true" : "false", zinfo->cacheMissMaxRecords);
    
    // modified by wei wu on 251211: PC access recording configuration
    zinfo->pcAccessRecordingEnabled = config.get<bool>("sim.pcAccessRecording.enabled", false);
    zinfo->pcAccessMaxRecords = config.get<uint64_t>("sim.pcAccessRecording.maxRecords", 10000000UL);
    
    info("PC access recording configuration: enabled=%s, maxRecords=%lu", 
         zinfo->pcAccessRecordingEnabled ? "true" : "false", zinfo->pcAccessMaxRecords);
    ...
    // modified by wei wu on 250816: Initialize cache miss recorder manager
    g_cacheMissRecorderManager = new CacheMissRecorderManager();
    g_cacheMissRecorderManager->init(zinfo->cacheMissRecordingEnabled, zinfo->cacheMissMaxRecords);
    
    // modified by wei wu on 251211: Initialize PC access recorder manager
    g_pcAccessRecorderManager = new PCAccessRecorderManager();
    g_pcAccessRecorderManager->init(zinfo->pcAccessRecordingEnabled, zinfo->pcAccessMaxRecords);

    
    // // Initialize fake hit manager
    // g_fakeHitManager = new FakeHitManager();
    // info("Fake hit manager initialized");
