 ...
 */
enum UopType : uint8_t {UOP_GENERAL, UOP_LOAD, UOP_STORE, UOP_STORE_ADDR, UOP_FENCE, UOP_OVEC, UOP_SW_PREFETCH}; // modified by wei on 251010: add UOP_SW_PREFETCH for software prefetch instructions
...
struct DynBbl {
    ...
    // Standard allocation: [DynBbl header][DynUop uop[uops]]
    static uint32_t bytes(uint32_t uops) {
        return offsetof(DynBbl, uop) + sizeof(DynUop)*uops /*wtf... offsetof doesn't work with uop[uops]*/;
    }

    // Extended allocation: [DynBbl header][DynUop uop[uops]][uint64_t uopPC[uops]]
    // uopPC[i] stores the x86 instruction PC that uop[i] was decoded from.
    static uint32_t bytes_with_pc(uint32_t uops) {
        return bytes(uops) + sizeof(uint64_t) * uops;
    }

    // Returns pointer to the per-uop PC array (stored immediately after uop[uops]).
    // Only valid when allocated with bytes_with_pc().
    uint64_t* uopPCArray() {
        return reinterpret_cast<uint64_t*>(&uop[uops]);
    }
    const uint64_t* uopPCArray() const {
        return reinterpret_cast<const uint64_t*>(&uop[uops]);
    }
...
class Decoder {
    ...
    private:
        ...
        static void emitSwPrefetch(Instr& instr, uint32_t idx, DynUopVec& uops);
        static void emitSwPrefetches(Instr& instr, DynUopVec& uops);
