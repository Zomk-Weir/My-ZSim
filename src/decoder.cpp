...
// modified by wei on 251010: add software prefetch emit functions
void Decoder::emitSwPrefetch(Instr& instr, uint32_t idx, DynUopVec& uops) {
    assert(idx < instr.numLoads);
    uint32_t op = instr.loadOps[idx];
    uint32_t baseReg = INS_OperandMemoryBaseReg(instr.ins, op);
    uint32_t indexReg = INS_OperandMemoryIndexReg(instr.ins, op);
    ...
    uop.rs[0] = baseReg;
    uop.rs[1] = indexReg;
    uop.rd[0] = 0; // Software prefetch doesn't write to any register
    uop.type = UOP_SW_PREFETCH;
    uop.lat = 1; // Fast completion, don't wait for memory
    uop.portMask = PORT_2; // Use load port
    uops.push_back(uop);
}

void Decoder::emitSwPrefetches(Instr& instr, DynUopVec& uops) {
    for (uint32_t i = 0; i < instr.numLoads; i++) {
        emitSwPrefetch(instr, i, uops);
    }
}
...
bool Decoder::decodeInstr(INS ins, DynUopVec& uops) {
    ...
    switch (category) {
        ...
        case XC(DATAXFER):
            switch (opcode) {
                ...
                case XO(XCHG):
                    ...
                    //     uint32_t orient = static_cast<uint32_t>(lastOvecOperand & 0xFFFFFFFF);  //what abput the base addr of Env variable?? why you add the pointer of Env variable in the source code??
                    ...
                    //         if (_blockAddr == lastCacheLine) {
                    //             // info("same block address, skipping"); 
                    //             continue;
                    //         }

                    //         ovecLoadAddresses.push(_addr);
                    //         // info("OVEC LOAD hint from decoding xchg, the generated address is: %ld", _addr);
                    ...
                    //we disable the ovec load hint for xchg, modified by wuwei
                    emitXchg(instr, uops);
                    break;
                default:
        ...
        // AVX2 integer/permute/shift instructions (added by wei)
        // Latency/port data from Agner Fog's Haswell instruction tables.
        case XC(AVX2):
            {
                uint32_t lat = 1;
                uint8_t ports = PORTS_015;
                switch (opcode) {
                    // --- Lane insert/extract ---
                    // vinserti128: insert xmm into ymm[127:0] or ymm[255:128]
                    // Haswell: 3-cycle lat, PORT_5
                    case XO(VINSERTI128):
                        lat = 3; ports = PORT_5;
                        break;

                    // --- Packed integer arithmetic ---
                    // vpsubq: 256-bit packed qword subtract
                    // Haswell: 1-cycle lat, PORT_0|PORT_5
                    case XO(VPSUBQ):
                        lat = 1; ports = PORT_0 | PORT_5;
                        break;

                    // --- Packed integer comparison ---
                    // vpcmpeqd: 256-bit packed dword equality compare -> mask
                    // Haswell: 1-cycle lat, PORT_0|PORT_5
                    case XO(VPCMPEQD):
                        lat = 1; ports = PORT_0 | PORT_5;
                        break;

                    // vpcmpgtq: 256-bit packed qword signed greater-than compare
                    // Haswell: 3-cycle lat, PORT_1 (same as 128-bit PCMPGTQ)
                    case XO(VPCMPGTQ):
                        lat = 3; ports = PORT_1;
                        break;

                    // --- Zero/sign extension ---
                    // vpmovzxdq: zero-extend 4 x dword (xmm) -> 4 x qword (ymm)
                    // Haswell: 3-cycle lat, PORT_5 (shuffle/permute unit)
                    case XO(VPMOVZXDQ):
                        lat = 3; ports = PORT_5;
                        break;

                    // --- Variable-count vector shifts ---
                    // vpsrlvq: 256-bit packed qword logical right shift by variable count
                    // Haswell: 3-cycle lat, PORT_0
                    case XO(VPSRLVQ):
                        lat = 3; ports = PORT_0;
                        break;

                    // vpsllvq: 256-bit packed qword logical left shift by variable count
                    // Haswell: 3-cycle lat, PORT_0
                    case XO(VPSLLVQ):
                        lat = 3; ports = PORT_0;
                        break;
                    ...
                    // --- Permute ---
                    // vpermd: permute 256-bit packed dwords using dword indices
                    // Haswell: 3-cycle lat, PORT_5
                    case XO(VPERMD):
                        lat = 3; ports = PORT_5;
                        break;
                    ...
                    default:
                        // Other AVX2 instructions: approximate as 1-cycle ALU op
                        inaccurate = true;
                }
                emitBasicOp(instr, uops, lat, ports);
            }
            break;

        // AVX logical-FP instructions (vxorps, vorps, vandps, etc.)
        // These share XED_CATEGORY_LOGICAL_FP with their non-VEX counterparts.
        // Haswell: 1-cycle lat, PORT_0|PORT_5
        case XC(LOGICAL_FP):
            emitBasicOp(instr, uops, 1, PORT_0 | PORT_5);
            break;
        ...
        /* Prefetches */
        case XC(PREFETCH):
            // emitLoads(instr, uops); // original code
            // modified by wei on 251010: software prefetch should be handled differently from regular loads
            // Software prefetch instructions should complete quickly without waiting for memory response
            emitSwPrefetches(instr, uops);
            break;
        ...
        case XC(AVX2GATHER):
            // modified by wei: model vpgatherdq as N independent UOP_LOADs.
            // vpgatherdq ymm_dst, [base + xmm_idx * scale], ymm_mask
            //   256-bit dst: 4 x 64-bit elements, each from a separate scatter address.
            // We decompose this into 4 UOP_LOAD uops that consume the 4 load addresses
            // injected by GatherDQ256LoadElements() in zsim.cpp (via IndirectPredLoadSingle).
            // This keeps the loadIdx == loads invariant in OOOCore::bbl() satisfied.
            // Inactive lanes (mask sign bit == 0) are injected as -1L (predFalseLoad path),
            // which OOOCore::bbl() handles as a fast-complete no-cache-access load.
            // Latency: Haswell vpgatherdq is ~5 cycles/element on PORT_2.
            if (opcode == XO(VPGATHERDQ)) {
                // info("decoding VPGATHERDQ ins: %s", INS_Disassemble(instr.ins).c_str());
                assert(instr.numLoads >= 1);
                uint32_t memOp   = instr.loadOps[0];
                uint32_t baseReg = INS_OperandMemoryBaseReg(instr.ins, memOp);
                uint32_t idxReg  = INS_OperandMemoryIndexReg(instr.ins, memOp);
                // dst ymm register (first written operand)
                uint32_t dstReg  = (instr.numOutRegs > 0) ? instr.outRegs[0] : 0;
                // Each of the 4 UOP_LOADs writes to the same dst ymm.
                // The scoreboard will take the max ready-time across all 4, which is
                // correct: the result is not usable until all lanes have completed.
                for (int i = 0; i < 4; i++) {
                    DynUop uop;
                    uop.clear();
                    uop.rs[0]    = baseReg;   // base reg dependency
                    uop.rs[1]    = idxReg;    // index reg (xmm) dependency
                    uop.rd[0]    = (i == 0) ? dstReg : REG_LOAD_TEMP; // only first writes real dst; rest use temp to avoid false WAW stalls
                    uop.lat      = 5;         // Haswell: ~5 cycles per gather element
                    uop.type     = UOP_LOAD;
                    uop.portMask = PORT_2;
                    uops.push_back(uop);
                }
            } else {
                // Other  AVX2 gather variants: approximate as a single slow load
                // info("decoding AVX2Gather ins: %s", INS_Disassemble(instr.ins).c_str());                
                emitBasicOp(instr, uops, 5, PORT_2);
                inaccurate = true;
            }
            break;

        case XC(GATHER):
            // SSE/AVX gather (non-AVX2): approximate
            // info("GATHER ins: %s", INS_Disassemble(instr.ins).c_str());
            emitBasicOp(instr, uops, 5, PORT_2);
            inaccurate = true;
            break;

        default: {}
            // info("Invalid instruction category: %s", INS_Disassemble(instr.ins).c_str());
    }
...
BblInfo* Decoder::decodeBbl(BBL bbl, bool oooDecoding) {
    ...
    if (oooDecoding) {
        ...
        for (uint32_t i = 0; i < instrs; i++) {
            ...
            // if (psz > 128 /*leftover*/|| pcnt > 64 /*max predecs*/) {  //try to entend the precode bw
                psz = bytes;
                pcnt = 1;
                pblk = block;
                pcyc++;
        ...
        //2. Decoding
        //4-1-1-1 rules: One big decoder and 3 small decoders. Small decoders can only take instructions that produce 1 uop AND are at most 7 bytes long
        uint32_t uopIdx = 0;
        ...
        //Allocate (use bytes_with_pc to reserve space for per-uop PC array after uop[])
        uint32_t objBytes = offsetof(BblInfo, oooBbl) + DynBbl::bytes_with_pc(uopVec.size());
        ...
        // Fill per-uop PC array: uopPC[i] = PC of the x86 instruction that produced uop[i].
        // instrUops[i] == 0 means the instruction was fused into the previous one (skip it).
        {
            uint64_t* uopPC = dynBbl.uopPCArray();
            uint32_t uopOffset = 0;
            for (uint32_t i = 0; i < instrUops.size(); i++) {
                if (instrUops[i] == 0) continue;  // fused branch: no uops emitted
                for (uint32_t j = 0; j < instrUops[i]; j++) {
                    uopPC[uopOffset + j] = (uint64_t)instrAddr[i];
                }
                uopOffset += instrUops[i];
            }
            assert(uopOffset == dynBbl.uops);
        }
