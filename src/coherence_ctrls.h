...
class MESIBottomCC : public GlobAlloc {
    private:
        ...
        // modified by wei wu on 250816: Cache reference for miss recording
        Cache* parentCache;
    ...
    public:
        MESIBottomCC(uint32_t _numLines, uint32_t _selfId, bool _nonInclusiveHack) : numLines(_numLines), selfId(_selfId), nonInclusiveHack(_nonInclusiveHack) {
            ...
            futex_init(&ccLock);
            // modified by wei wu on 250816: Initialize cache reference
            parentCache = nullptr;
        }

        void init(const g_vector<MemObject*>& _parents, Network* network, const char* name);
        
        // modified by wei wu on 250816: Set parent cache reference
        void setParentCache(Cache* cache) { parentCache = cache; }
...
class MESICC : public CC {
    ...
    public:
        ...
        MESICC(uint32_t _numLines, bool _nonInclusiveHack, g_string& _name) : tcc(nullptr), bcc(nullptr),
            numLines(_numLines), nonInclusiveHack(_nonInclusiveHack), name(_name), parentCache(nullptr) {}
        ...
        void setParents(uint32_t childId, const g_vector<MemObject*>& parents, Network* network) {
            ...
            // modified by wei wu on 250816: Set cache reference after bcc is created
            if (parentCache) {
                bcc->setParentCache(parentCache);
            }
        }
        ...
        // modified by wei wu on 250816: Set parent cache reference for miss recording
        void setParentCache(Cache* cache) {
            parentCache = cache;
            if (bcc) {
                bcc->setParentCache(cache);
            }
        }
        ...
        uint64_t processAccess(const MemReq& req, int32_t lineId, uint64_t startCycle, uint64_t* getDoneCycle = nullptr) {
            ...
            if (lineId == -1 || (((req.type == PUTS) || (req.type == PUTX)) && !bcc->isValid(lineId))) { //can only be a non-inclusive wback
                ...
                assert(!isPrefetch || req.type == GETS || req.type == GETX);//modified by WeiWu on 250901 to support prefetching GETX requests;                                                                                               
...
class MESITerminalCC : public CC {
    ...
    public:
        //Initialization
        MESITerminalCC(uint32_t _numLines, const g_string& _name) : bcc(nullptr), numLines(_numLines), name(_name), parentCache(nullptr) {}
