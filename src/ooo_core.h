...
class OOOCore : public Core {
    private:
        ...
        Address loadAddrs[256]; //modified by wei on 251112: to expand the load address size,initially set to 256
        Address loadPCs[256]; //modified by wei on 251112: to expand the load PC size,initially set to 256

        Address storeAddrs[256]; //modified by wei on 251112: to expand the store address size,initially set to 256
        Address storePCs[256]; //modified by wei on 251112: to expand the store PC size,initially set to 256
        uint32_t loads;
        uint32_t stores;
        ...
        ReorderBuffer<32, 4> loadQueue; //modified by wei on 251112: to expand the load queue size,initially set to 32,4
        ReorderBuffer<32, 4> storeQueue; //modified by wei on 251112: to expand the store queue size,initially set to 32,4
        ...
        ReorderBuffer<128, 4> rob;   //modified by wei on 251112: to expand the ROB size,initially set to 128,4
        ...
        CycleQueue<28> uopQueue;  //modified by wei on 251112: to expand the uop queue size,initially set to 28

        uint64_t instrs, uops, bbls, approxInstrs, mispredBranches, BranchCount;
        ...
        // ---- Cross-BBL state for PC-range timing tracking ----
        // Must be member variables (not locals in bbl()) so they survive across BBL calls.
        uint64_t LineChkDispatchCycle; // dispatch of the "start" instruction
        uint64_t LineChkCommitCycle;   // commit  of the "end"   instruction

        // CSV file handle for recording all discrete consumed/overlap cycle values.
        // Opened in constructor, written per-event with fflush, closed in destructor.
        // File is named "<core_name>_pc_track.csv" in the working directory.
        FILE* pcTrackCsvFile;

        // True while we are inside the "after 0x406073, before 0x406090" window.
        // Set to true when 0x406073 is flushed; cleared when 0x406090 begins.
        bool pcRangeActive;
